...
void Map::createRoom(bool first, int x1, int y1, int x2, int y2)
{
    ...
    if (first) // Causes the program to render two players????
    {
        gameLoop.player->x = (x1 + y1) / 2;
        gameLoop.player->y = (x1 + y1) / 2;
    ...
    {   //Very broken - actors often spawn inside walls.
        ...
        if (rng->getInt(0, 3) == 0)
        {
            Actor *actor = new Actor((x1 + x2) / 2, (y1 + y2) / 2, '@', TCODColor::yellow);
            actor->render();
            actors.push(actor);
            delete actor;
        }
